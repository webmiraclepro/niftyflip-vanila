class Router {
  ...
  loadRoute(...urlSegments) {
    ...
    if (url == "/") {
      ...
      let checkoutItem = document.getElementById("items_checkout")
      let item = ""
      // Chatbox hide and show function
      openChatBox.addEventListener("click", () => {
        if (getComputedStyle(chatBox).right == "0px") {
      ...
      let openCheckoutFunc = () => {
        if (checkout.style.display == "none") checkout.style.display = "block"
        ...
        console.log(item)
        checkoutItem.innerHTML = item
        withDrawPanel.style.width = "calc(100% - 260px)"
      }

      // Open checkbox
      for (let i = 0; i < cardBtn.length; i++) {
        cardBtn[i].addEventListener("click", openCheckoutFunc, false)
      }
